...
n = 100                  # TUNABLE: bucket size
pairs_we_care_about = ['KG','KA','BG','KH','WG']   # TUNABLE
...
for g in season2:
    ...
    if g["usernames"][0] != "cptKav" and g["usernames"][0] != "probablytom":
        s1meta[bucket][pair1] += 1
    if g["usernames"][1] != "cptKav" and g["usernames"][1] != "probablytom":
        s1meta[bucket][pair2] += 1
    count+=1
    if count == n:
        count = 0
        bucket += 1
...
pairs_we_care_about = []
for p in pairs:
    if s1meta[max(s1meta.keys())-1][p]/60 > 0.15:
        pairs_we_care_about += [p]

for p in pairs_we_care_about:
        values_to_plot = [s1meta[x][p]/n for x in s1meta.keys()]
        ax.plot([x*n for x in list(s1meta.keys())][:-1], values_to_plot[:-1], label = p)
for p in pairs:
    if p not in pairs_we_care_about:
        values_to_plot = [s1meta[x][p]/n for x in s1meta.keys()]
        ax.plot([x*n for x in list(s1meta.keys())][:-1], values_to_plot[:-1], alpha=0.1, color="grey")

plt.title("Meta comparison of particular pairs: " + str(pairs_we_care_about) + " with bucket size of " + str(n))
plt.xlabel("Games played")
plt.ylabel("proportion of times pair was picked")
plt.legend()
plt.tight_layout()
plt.show()
