...
colours = ["lightblue","tomato","bisque","forestgreen","blue","red","orange","pink","cyan","coral","magenta","lightslategray","crimson","thistle","darkviolet","mediumpurple","moccasin","seagreen","darkgray","brown","olivedrab","aquamarine", "sienna","orange","deepskyblue","cadetblue","indigo","gold"]
...
G = nx.DiGraph()
for p in pairs:
    G.add_node(p, color = colours[pairs.index(p)])

char_is_best = [False]*8
...
for i in range(len(pairs)):
    ...
    for j in range(len(pairs)):
        if tango_optimality[j][i] > opt_v:
            opt_v = tango_optimality[j][i]
            counter = pairs[j]
    
    G.add_edge(pair, counter, weight=opt_v, color = colours[pairs.index(counter)])
    for c in counter:
        if not char_is_best[chars.index(c)]:
            char_is_best[chars.index(c)] = True
            print(c, "is best against", pairs[i], "in", counter)

print("s2:")
for i in range(len(chars)):
    print("{0} is optimal against something... {1}".format(chars[i], char_is_best[i]))

weights = [(x-0.4)*25.0 for x in nx.get_edge_attributes(G,'weight').values()]
...
for i in range(len(pairs)):
    ...
    counter = pair
    for j in range(len(pairs)):
        if beta_optimality[j][i] > opt_v:
            opt_v = beta_optimality[j][i]
            counter = pairs[j]
...
print("CYCLE S1: {0}".format(nx.find_cycle(G)))
...
nx.draw(G, pos, ax=axs[0,0],
        ...
        with_labels=True,
        arrowstyle="-")
axs[0,0].set_title("Initial (optimality)", fontsize=18)
axs[0,0].axis('off')
axs[0,1].axis('off')
axs[0,1].set_title("Updated (optimality)", fontsize=18)


# CREATE S1 effective vs graphs.
G = nx.DiGraph()
for p in pairs:
    G.add_node(p, color = colours[pairs.index(p)])
...
nx.draw(G, pos, ax=axs[1,0],
        ...
        with_labels=True,
        arrowstyle="-")
axs[1,0].set_title("Initial (effective @ >=0.7)", fontsize=18)
axs[1,0].axis('off')

# CREATE S2 effective vs graphs.
G = nx.DiGraph()
for p in pairs:
    G.add_node(p, color = colours[pairs.index(p)])
...
nx.draw(G, pos, ax=axs[1,1],
        ...
        with_labels=True,
        arrowstyle="-")
axs[1,1].set_title("Updated (effective @ >=0.7)", fontsize=18)
axs[1,1].axis('off')
