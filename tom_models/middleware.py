...
MIDDLEWARE_CHANGESET = "initial beta release"

CHARACTER_ORDERING = ['K', 'A', 'R', 'H', 'W', 'B', 'M', 'G']
MAX_HEALTHS = [10, 8, 8, 10, 8, 10, 7, 8]
...
def login():
    ...
    try:
        try:
            ...
            print('got user')
            # If the user hasn't logged in yet today, give them 20 points.
            ...
            if delta_days == 1:
                if player['badge_progressions'][BADGE_CONSECUTIVE_LOGINS] == player['badge_progressions'][
                    BADGE_CONSECUTIVE_LOGINS + 1]:
    ...
    try:
        ...
        if 'most recent activity' in abandoned_game:
            ...
            if (datetime.now() - abandoned_game['most recent activity'][0]).days < 1 or \
                ...
                delta = calc_delta(user['skill_points'],
                                   opponent['skill_points'],
                                   False)
                user['skill_points'] = max(0, user['skill_points'] + delta)
...
def game_over():
    try:
        ...
        if user_won_game != "Abandon":
            ...
            delta = calc_delta(user.get('skill_points', 0), opponent.get('skill_points', 0), user_won_game)
            ...
            if len(game["most recent activity"]) > 2 and game["most recent activity"][2] == "apply abandon penalty":
                delta = calc_delta(user['skill_points'], opponent['skill_points'], won=True)
...
def leaderboard(username, allskills=None):
    try:
        ...
        return {'ranks': ranked_skills}, 200
...
def update_badges_chosen_from_lists(player, opp):
...
def motd(username):
    try:
        ...
        return random.choice(["Thanks for playing RPGLite, " + username + "!",
                               ...
                               'Remember: if you want to play against new people, why not match against them from the "New Users" list?',
