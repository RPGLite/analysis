...
class ShepherdConfig:
    def __init__(self):
        ...
        self.game_filters.append(lambda game: game["_id"] != objectid.ObjectId("5e98b4658a225cfc82573fd1"))  # This game doc is broken TODO: just remove the broken game doc from the cache on load
...
custom_pure, clear_pure_cache = clevercache()


def game_doc_was_abandoned(gamedoc):
    return (gamedoc.get('p1_abandon', False)
            or gamedoc.get('p2_abandon', False)
            or 'apply abandon penalty' in gamedoc['most recent activity']
            or gamedoc['active_player'] == 'none'
            or not gamedoc['p1_selected']
            or not gamedoc['p2_selected']
            or 'winner' not in gamedoc
            or 'Moves' not in gamedoc
            or len(gamedoc['Moves']) == 0)
...
class Shepherd:
    ...
    def __init__(self, load_cache_by_file=True, config=None):
    ...
    def filtered_games(self):
        ...
        if self.config.filter_games_by_users:
            ...
            def player_1_valid(game):
                return game.get('usernames', ['none', 'none'])[0] in valid_users_by_username

            def player_2_valid(game):
                return game.get('usernames', ['none', 'none'])[1] in valid_users_by_username
